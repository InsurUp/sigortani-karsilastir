        ...
        for (const company of agencyConfig.homepage.partners.companies) {
          if (company.products) {
            for (const [branchKey, productIdsInConfig] of Object.entries(company.products)) {
              if (Array.isArray(productIdsInConfig)) {
                // Hem array formatını ([3188, 3279]) hem obje formatını ([{id: 3188, displayMode: "FULL_ACCESS"}]) destekle
                const hasProduct = productIdsInConfig.some(product => {
                  if (typeof product === 'number') {
                    // Eski array format: [3188, 3279]
                    return product === configProductId;
                  } else if (typeof product === 'object' && product !== null && 'id' in product) {
                    // Yeni obje format: [{id: 3188, displayMode: "FULL_ACCESS"}]
                    return (product as any).id === configProductId;
                  }
                  return false;
                });
                
                if (hasProduct) {
                  resolvedBranch = branchKey.toLowerCase();
                  break;
                }
              }
            }
