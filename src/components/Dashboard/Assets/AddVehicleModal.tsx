...
const AddVehicleModal: React.FC<AddVehicleModalProps> = ({ onClose, initialData, onSuccess }) => {
  ...
  const handleChange = (name: string, value: string) => {
    ...
    // Eski Poliçe Bilgileri için validasyonlar
    if ([
      'oldPolicyNo',
      'oldPolicyRenewalNo',
      'oldPolicyInsuranceCompanyNo',
      ...
      setFormData((prev) => ({ ...prev, [name]: sanitizedValue }));
      let errorMsg = '';
      if (name === 'oldPolicyNo' && sanitizedValue.length > 0) {
        if (sanitizedValue.length < 12 || sanitizedValue.length > 16) {
          errorMsg = 'Eski Poliçe No 12 ile 16 hane arasında olmalıdır.';
        }
      }
      if (name === 'oldPolicyRenewalNo' && sanitizedValue.length > 0) {
        if (sanitizedValue.length !== 1) {
          errorMsg = 'Yenileme numarası yalnızca 1 haneli bir sayı olmalıdır (0, 1, 2 gibi).';
        }
      }
      if (name === 'oldPolicyInsuranceCompanyNo' && sanitizedValue.length > 0) {
        if (sanitizedValue.length < 1 || sanitizedValue.length > 2) {
          errorMsg = 'Sigorta şirketi numarası 1 veya 2 haneli olmalıdır.';
        }
      }
      if (name === 'oldPolicyAgencyNo' && sanitizedValue.length > 0) {
    ...
    <div className="fixed inset-0 z-999 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div className="max-h-[95vh] w-full max-w-4xl overflow-y-auto rounded-xl bg-white p-4 md:p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
          ...
          <div>
            <h5 className="mb-3 font-medium text-gray-900 text-sm md:text-base">Eski Poliçe Bilgileri</h5>
            <div className="grid grid-cols-1 gap-4 md:gap-6 md:grid-cols-2">
              <div>
                <Input
                  label="Eski Poliçe No"
                  value={formData.oldPolicyNo}
                  onChange={(e) => handleChange('oldPolicyNo', e.target.value)}
                />
                {errors.oldPolicyNo && (
                  <p className="mt-1 text-xs text-red-500">{errors.oldPolicyNo}</p>
                )}
              </div>

              <div>
                <Input
                  label="Eski Poliçe Yenileme Numarası"
                  value={formData.oldPolicyRenewalNo}
                  onChange={(e) => handleChange('oldPolicyRenewalNo', e.target.value)}
                />
                {errors.oldPolicyRenewalNo && (
                  <p className="mt-1 text-xs text-red-500">{errors.oldPolicyRenewalNo}</p>
                )}
              </div>

              <div>
                <Input
                  label="Eski Poliçe Sigorta Şirketi No"
                  value={formData.oldPolicyInsuranceCompanyNo}
                  onChange={(e) => handleChange('oldPolicyInsuranceCompanyNo', e.target.value)}
                />
                {errors.oldPolicyInsuranceCompanyNo && (
                  <p className="mt-1 text-xs text-red-500">{errors.oldPolicyInsuranceCompanyNo}</p>
                )}
              </div>

              <div>
                <Input
                  label="Eski Poliçe Acenta No"
                  value={formData.oldPolicyAgencyNo}
                  onChange={(e) => handleChange('oldPolicyAgencyNo', e.target.value)}
                />
                {errors.oldPolicyAgencyNo && (
                  <p className="mt-1 text-xs text-red-500">{errors.oldPolicyAgencyNo}</p>
                )}
              </div>
            </div>
          </div>
          ...
          {/* Dain-i Mürtehin */}
          <div>
            <h5 className="mb-3 font-medium text-gray-900 text-sm md:text-base">Dain-i Mürtehin</h5>
