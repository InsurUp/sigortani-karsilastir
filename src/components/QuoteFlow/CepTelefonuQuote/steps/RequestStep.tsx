...
const RequestStep = ({ onNext, onBack, isFirstStep, isLastStep }: RequestStepProps) => {
  ...
  useEffect(() => {
    ...
    const errorFromStorage = localStorage.getItem('ceptelefonuRequestError');
    const successFromStorage = localStorage.getItem('ceptelefonuRequestSuccess');

    if (successFromStorage) {
      setRequestResult('success');
      setRequestCreated(true);
      localStorage.removeItem('ceptelefonuRequestSuccess');
    ...
    return (
      <Box sx={{ textAlign: 'center', py: 4, paddingBottom: 10 }}>
        ...
        <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
          Cep Telefonu Sigortası teklif talebiniz sisteme kaydedildi.<br />
  ...
  if (requestResult === 'existing') {
    return (
      <Box sx={{ textAlign: 'center', py: 4, paddingBottom: 10 }}>
        ...
        <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
          Cep Telefonu Sigortası için mevcut bir talebiniz bulunuyor.<br /> 
