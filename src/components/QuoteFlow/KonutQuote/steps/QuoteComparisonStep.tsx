...
// Konut için tüm olası teminat alanlarının etiket eşlemeleri
const KONUT_GUARANTEE_LABELS: Record<string, string> = {
  ...
  yangin: 'Yangın / Yıldırım / İnfilak',
  hirsizlik: 'Hırsızlık',
  deprem: 'Deprem ve Yanardağ Püskürmesi',
  selSu: 'Sel ve Su Baskını',
  firtina: 'Fırtına',
  yerKaymasi: 'Yer Kayması',
  karAgirligi: 'Kar Ağırlığı',
  dahiliSu: 'Dahili Su',
  duman: 'Duman',
  grevLokavt: 'Grev, Lokavt, Kargaşalık, Halk Hareketleri',
  teror: 'Terör',
  ferdiKaza: 'Ferdi Kaza',
  ucuncuSahisMaliMesuliyet: '3. Şahıs Mali Mesuliyet',
  kiraKaybi: 'Kira Kaybı',
  enkazKaldirma: 'Enkaz Kaldırma',
  elektrikHasarlari: 'Elektrik Hasarları',
  alternatifIkameGiderleri: 'Alternatif İkamet Giderleri',
  komsulukMaliMesuliyet: 'Komşuluk Mali Mesuliyet',
  evAsistans: 'Ev Asistans Hizmetleri',
  camKirilmasi: 'Cam Kırılması',
  kasaHirsizlik: 'Kasa Hırsızlık',
  misafirEsyalari: 'Misafir Eşyaları',
...
const convertKonutCoverageToGuarantees = (coverage: KonutCoverage | null): Guarantee[] => {
  ...
  // Tüm alanları dön ve Guarantee array'ine dönüştür
  Object.entries(coverage).forEach(([key, value]) => {
    // Teknik alanları atla
  ...
  // Bina bedelini her zaman en başa al (eğer varsa)
  return guarantees.sort((a, b) => {
    if (a.label === 'Bina Bedeli') return -1;
    if (b.label === 'Bina Bedeli') return 1;
    return 0;
  });
};
...
export default function QuoteComparisonStep({
  ...
  const processQuotesData = (quotesData: Quote[], currentCompanies: InsuranceCompany[]): ProcessedQuote[] => {
    return quotesData.map((quote: Quote) => {
      ...
      // --- Fallback: Önceki alan bazlı coverage birleştirme mantığı (Backend verisi yoksa) ---
      ...
      const mergedCoverage: any = { $type: 'konut' };
      ...
      allCoverageKeys.forEach(key => {
        ...
        // Eğer hiçbirinde geçerli değer bulunamadıysa, son çare olarak birinden al
      ...
      mergedCoverage.productBranch = quote.pdfCoverage?.productBranch || quote.insuranceServiceProviderCoverage?.productBranch || quote.initialCoverage?.productBranch || '';
