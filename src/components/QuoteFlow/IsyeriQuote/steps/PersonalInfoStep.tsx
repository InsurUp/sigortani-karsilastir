      ...
      try {
        ...
        if (!fullName || !city || !district) {
          ...
          if (isCoreDataComplete) {
            ...
            if (currentCustId) {
              localStorage.setItem('isyeriProposalId', currentCustId);
            }
  ...
  const formik = useFormik<FormikValues>({
    ...
    onSubmit: async (values) => {
      ...
      if (!showAdditionalInfo && values.email && values.job !== Job.Unknown) {
        localStorage.setItem('isyeriInitialEmail', values.email);
        localStorage.setItem('isyeriInitialJob', values.job.toString());
      }
      ...
      try {
        ...
        const emailFromLocalStorage = localStorage.getItem('isyeriInitialEmail');
        if (emailFromLocalStorage) {
          updatePayload.primaryEmail = emailFromLocalStorage;
        }
        
        // Job'u localStorage'dan al ve Unknown değilse gönder
        const jobFromLocalStorage = localStorage.getItem('isyeriInitialJob');
        ...
        if (currentCustomerId) {
            localStorage.setItem('isyeriProposalId', currentCustomerId);
        } else {
        }
    ...
    try {
      ...
      if (meData) {
        // customerId kontrolü
          ...
          const storedEmail = localStorage.getItem('isyeriInitialEmail');
        const storedJob = localStorage.getItem('isyeriInitialJob');
  ...
  const handleUpdateAdditionalInfo = async (values: FormikValues) => {
    ...
    try {
      ...
      if (customerIdForProposal) {
        localStorage.setItem('isyeriProposalId', customerIdForProposal); 
      } else {
      }
  ...
  const handleCreateRequestInPersonalInfo = async (tokenToUse?: string) => {
    try {
      ...
      if (!currentCustomerId) {
        ...
        localStorage.setItem('isyeriRequestError', 'Müşteri bilgisi bulunamadı. Lütfen önceki adımları kontrol edin.');
        onNext();
        return;
      }
      ...
      if (!currentToken) {
        // Hata durumunu localStorage'a kaydet ve RequestStep'te göster
        localStorage.setItem('isyeriRequestError', 'Kimlik doğrulama sorunu. Lütfen sayfayı yenileyin.');
        onNext();
        return;
      }
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (errorData.errors && Array.isArray(errorData.errors) && errorData.errors.length > 0) {
            ...
            if (apiError.includes('zaten açık bir yeni satış fırsatı talebi bulunmaktadır')) {
              errorMessage = 'Bu müşteri için zaten açık bir İşyeri Sigortası talebi bulunmaktadır. Mevcut talebin sonuçlanmasını bekleyiniz.'; 
            } else {
        ...
        // Hata durumunu localStorage'a kaydet ve RequestStep'te göster
        localStorage.setItem('isyeriRequestError', errorMessage);
        onNext();
        return;
      }
      ...
      localStorage.setItem('isyeriRequestSuccess', 'true');
      ...
      // Hata durumunu localStorage'a kaydet ve RequestStep'te göster
      localStorage.setItem('isyeriRequestError', error instanceof Error ? error.message : 'Talep oluşturulurken bir hata oluştu'); 
      onNext();
    } finally {
      setIsLoading(false);
    }
      ...
      >
        ...
        >
          ...
          <Typography sx={{ mb: 3, fontSize: '1rem', color: 'text.secondary' }}>
            Kurumsal müşterilerimize İşyeri Sigortası hizmeti verememekteyiz.<br />
