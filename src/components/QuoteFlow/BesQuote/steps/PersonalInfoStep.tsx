      ...
      try {
        ...
        if (!fullName || !city || !district) {
          ...
          if (isCoreDataComplete) {
            ...
            if (currentCustId) {
              localStorage.setItem('besProposalId', currentCustId);
            }
  ...
  const formik = useFormik<FormikValues>({
    ...
    onSubmit: async (values) => {
      ...
      if (!showAdditionalInfo && values.email && values.job !== Job.Unknown) {
        localStorage.setItem('besInitialEmail', values.email);
        localStorage.setItem('besInitialJob', values.job.toString());
      }
      ...
      try {
        ...
        const emailFromLocalStorage = localStorage.getItem('besInitialEmail');
        if (emailFromLocalStorage) {
          updatePayload.primaryEmail = emailFromLocalStorage;
        }
        
        // Job'u localStorage'dan al ve Unknown değilse gönder
        const jobFromLocalStorage = localStorage.getItem('besInitialJob');
        ...
        if (currentCustomerId) {
            localStorage.setItem('besProposalId', currentCustomerId);
        } else {
        }

        // Step geçişi - analytics step event
        pushToDataLayer({
          event: "bes_formsubmit",
          form_name: "bes_step1"
        });
        onNext();
      } catch (error) {
    ...
    try {
      ...
      if (meData) {
        // customerId kontrolü
          ...
          const storedEmail = localStorage.getItem('besInitialEmail');
        const storedJob = localStorage.getItem('besInitialJob');
  ...
  const handleUpdateAdditionalInfo = async (values: FormikValues) => {
    ...
    try {
      ...
      if (customerIdForProposal) {
        localStorage.setItem('besProposalId', customerIdForProposal);
      } else {
      }
  ...
  const handleCreateRequestInPersonalInfo = async (tokenToUse?: string) => {
    try {
      ...
      if (!currentCustomerId) {
        ...
        localStorage.setItem('besRequestError', 'Müşteri bilgisi bulunamadı. Lütfen önceki adımları kontrol edin.');
        onNext();
        return;
      }
      ...
      if (!currentToken) {
        // Hata durumunu localStorage'a kaydet ve RequestStep'te göster
        localStorage.setItem('besRequestError', 'Kimlik doğrulama sorunu. Lütfen sayfayı yenileyin.');
        onNext();
        return;
      }
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (errorData.errors && Array.isArray(errorData.errors) && errorData.errors.length > 0) {
            ...
            if (apiError.includes('zaten açık bir yeni satış fırsatı talebi bulunmaktadır')) {
              errorMessage = 'Bu müşteri için zaten açık bir Bireysel Emeklilik Sigortası talebi bulunmaktadır. Mevcut talebin sonuçlanmasını bekleyiniz.';
            } else {
        ...
        // Hata durumunu localStorage'a kaydet ve RequestStep'te göster
        localStorage.setItem('besRequestError', errorMessage);
        onNext();
        return;
      }
      ...
      localStorage.setItem('besRequestSuccess', 'true');
      localStorage.removeItem('besRequestError');
      ...
      // Hata durumunu localStorage'a kaydet ve RequestStep'te göster
      localStorage.setItem('besRequestError', error instanceof Error ? error.message : 'Talep oluşturulurken bir hata oluştu');
      onNext();
    } finally {
      setIsLoading(false);
    }
      ...
      >
        ...
        >
          ...
          <Typography sx={{ mb: 3, fontSize: '1rem', color: 'text.secondary' }}>
            Kurumsal müşterilerimize Bireysel Emeklilik Sigortası hizmeti verememekteyiz.<br /> 
