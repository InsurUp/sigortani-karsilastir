...
const RequestStep = ({ onNext, onBack, isFirstStep, isLastStep }: RequestStepProps) => {
  ...
  useEffect(() => {
    ...
    const errorFromStorage = localStorage.getItem('seyahatRequestError');
    const successFromStorage = localStorage.getItem('seyahatRequestSuccess');

    if (successFromStorage) {
      setRequestResult('success');
      setRequestCreated(true);
      localStorage.removeItem('seyahatRequestSuccess');
      ...
      localStorage.removeItem('seyahatRequestError'); 
    }
  }, []);
    ...
    return (
      <Box sx={{ textAlign: 'center', py: 4, paddingBottom: 10 }}>
        ...
        <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
          Seyahat Sigortası teklif talebiniz sisteme kaydedildi.<br />
  ...
  if (requestResult === 'existing') {
    return (
      <Box sx={{ textAlign: 'center', py: 4, paddingBottom: 10 }}>
        ...
        <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
          Seyahat Sigortası için mevcut bir talebiniz bulunuyor.<br />
  ...
  return (
    <Box sx={{ textAlign: 'center', py: 4 }}>
      <Typography variant="h5" gutterBottom sx={{ mb: 3, fontWeight: 'bold' }}>
        Seyahat Sigortası Teklif Talebi
      </Typography>
