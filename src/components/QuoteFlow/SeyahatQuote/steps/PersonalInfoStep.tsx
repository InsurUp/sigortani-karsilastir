      ...
      try {
        ...
        if (!fullName || !city || !district) {
          ...
          if (isCoreDataComplete) {
            ...
            if (currentCustId) {
              localStorage.setItem('nakliyatProposalId', currentCustId);
            }
  ...
  const formik = useFormik<FormikValues>({
    ...
    onSubmit: async (values) => {
      ...
      if (!showAdditionalInfo && values.email && values.job !== Job.Unknown) {
        localStorage.setItem('nakliyatInitialEmail', values.email);
        localStorage.setItem('nakliyatInitialJob', values.job.toString());
      }
      ...
      try {
        ...
        const emailFromLocalStorage = localStorage.getItem('nakliyatInitialEmail');
        if (emailFromLocalStorage) {
          updatePayload.primaryEmail = emailFromLocalStorage;
        }
        
        // Job'u localStorage'dan al ve Unknown değilse gönder
        const jobFromLocalStorage = localStorage.getItem('nakliyatInitialJob');
        ...
        if (currentCustomerId) {
            localStorage.setItem('nakliyatProposalId', currentCustomerId);
        } else {
        }
        onNext();
      } catch (error) {
    ...
    try {
      ...
      if (meData) {
        // customerId kontrolü
          ...
          const storedEmail = localStorage.getItem('nakliyatInitialEmail');
          const storedJob = localStorage.getItem('nakliyatInitialJob');
  ...
  const handleUpdateAdditionalInfo = async (values: FormikValues) => {
    ...
    try {
      ...
      if (customerIdForProposal) {
        localStorage.setItem('nakliyatProposalId', customerIdForProposal);
      } else {
      }
  ...
  const handleCreateRequestInPersonalInfo = async (tokenToUse?: string) => {
    try {
      ...
      if (!currentCustomerId) {
        ...
        localStorage.setItem('nakliyatRequestError', 'Müşteri bilgisi bulunamadı. Lütfen önceki adımları kontrol edin.');
        onNext();
        return;
      }
      ...
      if (!currentToken) {
        // Hata durumunu localStorage'a kaydet ve RequestStep'te göster
        localStorage.setItem('nakliyatRequestError', 'Kimlik doğrulama sorunu. Lütfen sayfayı yenileyin.');
        onNext();
        return;
      }
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (errorData.errors && Array.isArray(errorData.errors) && errorData.errors.length > 0) {
            ...
            if (apiError.includes('zaten açık bir yeni satış fırsatı talebi bulunmaktadır')) {
              errorMessage = 'Bu müşteri için zaten açık bir Nakliyat Sigortası talebi bulunmaktadır. Mevcut talebin sonuçlanmasını bekleyiniz.';
            } else {
        ...
        // Hata durumunu localStorage'a kaydet ve RequestStep'te göster
        localStorage.setItem('nakliyatRequestError', errorMessage);
        onNext();
        return;
      }
      ...
      localStorage.setItem('nakliyatRequestSuccess', 'true');
      localStorage.removeItem('nakliyatRequestError');
      ...
      // Hata durumunu localStorage'a kaydet ve RequestStep'te göster
      localStorage.setItem('nakliyatRequestError', error instanceof Error ? error.message : 'Talep oluşturulurken bir hata oluştu');
      onNext();
    } finally {
      setIsLoading(false);
    }
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mt: 2 }}>
              ...
              <FormControlLabel
                ...
                label={
                  <Typography>
                    <Link
                      href="/ticari-elektronik-ileti-izni"
                      target="_blank"
                      underline="always"
                      sx={{ color: 'primary.main' }}
                    >
      ...
      >
        ...
        >
          ...
          <Typography sx={{ mb: 3, fontSize: '1rem', color: 'text.secondary' }}>
            Kurumsal müşterilerimize Nakliyat sigortası hizmeti verememekteyiz.<br /> 
